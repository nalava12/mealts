...
export namespace SchoolMenuParser {
  export function parse(rawData: string): SchoolMenu[] {
    if(rawData.length < 1)
      throw new Error('불러온 데이터가 올바르지 않습니다.');
    
    let monthlyMenu: SchoolMenu[] = [];
  ...
  function parseDay(rawData: string): SchoolMenu {
    let menu = new SchoolMenu();
